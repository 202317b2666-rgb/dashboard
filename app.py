...
# 3️⃣ Ensure iso_alpha column is string and remove missing values
hex_df = hex_df.dropna(subset=['iso_alpha'])
hex_df['iso_alpha'] = hex_df['iso_alpha'].astype(str)

# 4️⃣ Add a numeric column for testing colors
hex_df['TestValue'] = range(len(hex_df))

# 5️⃣ Remove any rows with missing color values
hex_df = hex_df.dropna(subset=['TestValue'])

# 6️⃣ Dummy line chart data for testing
...
fig = px.choropleth(
    ...
    locationmode='ISO-3'     # maps ISO alpha-3 codes
)
...
# 8️⃣ Capture click events
selected_points = plotly_events(fig, click_event=True)
st.plotly_chart(fig, use_container_width=True)

# 9️⃣ Show popup modal if a country is clicked
if selected_points:
